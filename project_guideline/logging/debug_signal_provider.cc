...
DebugSignalProvider::Create(const DebugSignalProviderOptions& options) {
  return absl::WrapUnique(new DebugSignalProvider(options));
}
...
DebugSignalProvider::~DebugSignalProvider() {
  ...
  //   auto status = Stop();
  //   if (!status.ok()) {
  //     LOG(ERROR) << "Shouter thread failed to stop: " << status;
  //   }
  // }
}

absl::Status DebugSignalProvider::Start() {
  // CHECK(shouter_thread_ == nullptr) << "Shouter thread is already started.";
  // if (options_.enable_data_shouting()) {
  //   running_ = true;
  //   shouter_thread_ = std::make_unique<std::thread>(
  //       absl::bind_front(&DataGuidelineShouter::RunShouter, this));
  // }
  return absl::OkStatus();
}

absl::Status DebugSignalProvider::Stop() {
  // CHECK(shouter_thread_) << "Shouter thread is already stopped.";
  // running_ = false;
  // shouter_thread_->join();
  // shouter_thread_ = nullptr;
  return absl::OkStatus();
}
...
void DebugSignalProvider::OnCameraPose(
  ...
  {
    absl::MutexLock lock(&camera_pose_mutex_);
...
void DebugSignalProvider::OnDetection(
  ...
  {
    absl::MutexLock lock(&detection_mutex_);
...
void DebugSignalProvider::OnGuideline(
  ...
  {
    absl::MutexLock lock(&guideline_mutex_);
...
void DebugSignalProvider::OnTrackingStateChanged(bool is_tracking) {
  {
    absl::MutexLock lock(&tracking_state_changed_mutex_);
...
void DebugSignalProvider::OnTrackingFeatures(
  ...
  {
    absl::MutexLock lock(&tracking_features_mutex_);
...
void DebugSignalProvider::NotifyDebugSignalCallbacks() {
  if (!options_.enable_debug_signal_provider()) {
    return;
  }
