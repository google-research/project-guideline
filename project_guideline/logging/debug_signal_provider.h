...
class DebugSignalProvider {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<DebugSignalProvider>> Create(
      const DebugSignalProviderOptions& options);
  ...
  void AddDebugSignalCallback(const DebugSignalCallback& debug_signal_callback)
      ABSL_LOCKS_EXCLUDED(debug_signal_callbacks_mutex_);

  absl::Status Start();
  absl::Status Stop();

 private:
  explicit DebugSignalProvider(const DebugSignalProviderOptions& options);
  void NotifyDebugSignalCallbacks()
      ABSL_LOCKS_EXCLUDED(debug_signal_callbacks_mutex_);
  void RunDebugSignalProvider();

 private:
  const DebugSignalProviderOptions options_;
  absl::Mutex camera_pose_mutex_;
  int64_t last_camera_pose_timestamp_us_ ABSL_GUARDED_BY(camera_pose_mutex_) =
      0;
  int64_t camera_pose_update_count_ ABSL_GUARDED_BY(camera_pose_mutex_) = 0;
  int64_t camera_pose_lag_count_ ABSL_GUARDED_BY(camera_pose_mutex_) = 0;
  int64_t current_camera_pose_fps_ ABSL_GUARDED_BY(camera_pose_mutex_) = 0;

  absl::Mutex detection_mutex_;
  int64_t last_detection_timestamp_us_ ABSL_GUARDED_BY(detection_mutex_) = 0;
  int64_t detection_update_count_ ABSL_GUARDED_BY(detection_mutex_) = 0;
  int64_t detection_lag_count_ ABSL_GUARDED_BY(detection_mutex_) = 0;
  int64_t current_detection_fps_ ABSL_GUARDED_BY(detection_mutex_) = 0;
  absl::Mutex tracking_features_mutex_;
  int64_t last_tracking_features_timestamp_us_
      ABSL_GUARDED_BY(tracking_features_mutex_) = 0;
  int64_t tracking_features_update_count_
      ABSL_GUARDED_BY(tracking_features_mutex_) = 0;
  int64_t tracking_features_lag_count_
      ABSL_GUARDED_BY(tracking_features_mutex_) = 0;
  int64_t current_tracking_features_fps_
      ABSL_GUARDED_BY(tracking_features_mutex_) = 0;

  absl::Mutex tracking_state_changed_mutex_;
  bool tracking_ ABSL_GUARDED_BY(tracking_state_changed_mutex_) = false;

  absl::Mutex guideline_mutex_;
  int64_t guideline_count_ ABSL_GUARDED_BY(guideline_mutex_) = 0;

  absl::Mutex debug_signal_callbacks_mutex_;
  std::vector<DebugSignalCallback> debug_signal_callbacks_
      ABSL_GUARDED_BY(debug_signal_callbacks_mutex_);
