...
#include "project_guideline/bazel/tools/filewrapper/test_embed_multiple_oss.h"
#include "project_guideline/bazel/tools/filewrapper/test_embed_single_oss.h"
...
TEST(FilewrapperOSSTest, EmbedSingle) {
  const EmbeddedFileToc* toc = test_embed_single_oss_create();

  EXPECT_EQ(test_embed_single_oss_size(), 1);
...
TEST(FilewrapperOSSTest, EmbedMultiple) {
  const EmbeddedFileToc* toc = testns::test_embed_multiple_oss_create();

  EXPECT_EQ(testns::test_embed_multiple_oss_size(), 3);
